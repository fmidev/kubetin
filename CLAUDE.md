...
An identity guard alone is NOT enough when the *same* key can have overlapping requests. The fleet detail panel learned this the hard way (#73 review): collapse + re-expand dispatches a second fetch for the same cluster, and the older in-flight result would pass a context-only guard and overwrite the newer one. `FleetDetailMsg{Seq, Result}` carries a generation from `m.fleet.detailSeq`; the receiver requires the newest. Corollary: every path that abandons a request must invalidate the identity or the generation. Clearing the identity is enough when the receiver can no longer match it — collapse and `leaveFleet` clear `m.fleet.expanded`; collapse deliberately keeps the detail state, stale `loading` included, which is safe only because every re-expansion either dispatches a fresh fetch (which resets it) or resets it explicitly. A path that leaves the same identity matchable *without* dispatching must do both itself — advance the generation and reset the detail state — as offline re-expansion does; it shipped exactly that bug before bumping `detailSeq`.
